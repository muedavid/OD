...
def viot_fusion_module(decoder_output, side, num_classes, num_filters_per_class, output_name="out_edge"):
    ...
    fusion_1 = tf.keras.layers.Concatenate(axis=-1)([decoder_output, side])
    fusion_2 = utils.convolution_block(fusion_1, BN=True, RELU=True, num_filters=2*num_filters, kernel_size=1)
    fusion_3 = utils.convolution_block(fusion_2, BN=True, RELU=True, num_filters=num_filters, kernel_size=3)
...
def shared_concatenation_and_classification(decoder_output, side, num_classes, num_filters_per_class, output_name):
    ...
    for i in range(num_classes):
        ...
        decoder_mult = utils.convolution_block(decoder_per_class, kernel_size=1, name="output_decoder_mult_{}".format(i),
                                         num_filters=2,
                                         RELU=False)
        ...
        decoder_concat = utils.convolution_block(decoder_per_class, kernel_size=1, name="output_decoder_concat_{}".format(i),
                                           num_filters=2)
