...
def decoder_small(decoder_input, output_dims, num_classes, num_filters_per_class):
    ...
    # x_1 = utils.convolution_block(x, num_filters=num_filters, kernel_size=3, dilation_rate=1, depthwise=True, BN=True,
    #                               RELU=False, name="decoder_1")
    return x


def lite_edge_decoder(daspp_input, side_input):
    daspp = utils.convolution_block(daspp_input, kernel_size=1, num_filters=20)
    
    daspp = tf.image.resize(daspp, (side_input.shape[1], side_input.shape[2]))
    
    decoder = tf.keras.layers.Concatenate()([daspp, side_input])
    decoder = utils.convolution_block(decoder, kernel_size=3, separable=True, num_filters=20)
    decoder = utils.convolution_block(decoder, kernel_size=3, separable=True, num_filters=20)
    decoder = utils.convolution_block(decoder, kernel_size=3, separable=True, num_filters=1)
