...
def sged_decoder(decoder_input, side, output_dims, num_filters=15, num_output_filters=10):
    ...
    side = utils.convolution_block(side, num_filters=1, kernel_size=1, dilation_rate=1,
                                   BN=True, RELU=True, name="decoder_1")
    
    x = tf.keras.layers.Concatenate(axis=-1)([x, side])
    
    x = utils.convolution_block(x, num_filters=num_filters, kernel_size=1, dilation_rate=1, seperable=True, BN=True,
...
def sged_side_feature(x, output_dims, num_filters, method="bilinear", name=None):
    x = utils.convolution_block(x, num_filters=num_filters, kernel_size=3, BN=True, RELU=True,
                                seperable=True,
                                name=name + "_1_conv3x3")
    x = utils.convolution_block(x, num_filters=num_filters, kernel_size=3, BN=True, RELU=True,
                                seperable=True,
                                name=name + "_2_conv3x3")
    x = utils.convolution_block(x, num_filters=1, kernel_size=1, BN=False, RELU=False,
                                name=name + "_conv1x1")
